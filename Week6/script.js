...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadButton.addEventListener('click', async function() {
        ...
        try {
            ...
            // YOUR CODE HERE:
            updateStatus('error', 'Failed to load data. Please check your connection.');
            loadButton.disabled = false; 
        ...
        restaurants.forEach(function(restaurant) {
            restaurantList.innerHTML += `
                <div class="restaurant-card">
                    ...
                    <p>Price: ${restaurant.priceRange}</p> <!-- Fixed this line -->
                </div>
            `;
        });
    });
        ...
        cheapRestaurants.forEach(function(restaurant) {
            filteredList.innerHTML += `
                <div class="restaurant-card">
                    <h3>${restaurant.name}</h3>
                    <p>Cuisine: ${restaurant.cuisine}</p>
                    <p>Price: ${restaurant.priceRange}</p> <!-- Fixed this line too -->
                </div>
            `;
        });   
        ...
        try {
            ...
            errorDisplay.innerHTML = `
                <div class="status-display error">
                    <p class="status-message">Error caught! The URL was bad, but our app didn't crash.</p>
                </div>
            `;
