...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    filterButton.addEventListener('click', (event) =>{
        ...
        cheapRestaurants.forEach((resto) => {
            filteredList.innerHTML += `<div>${resto.name} - ${resto.cuisine} (${resto.priceRange})</div>`;
        });
    ...
    findButton.addEventListener('click', (event) =>{
        ...
        if (bestRestaurant) {
            foundItem.innerHTML = `
                <div>
                    <strong>${bestRestaurant.name}</strong><br>
                    Cuisine: ${bestRestaurant.cuisine}<br>
                    Rating: ${bestRestaurant.rating} ⭐<br>
                    Price: ${bestRestaurant.priceRange}<br>
                    Neighborhood: ${bestRestaurant.neighborhood}
                </div>
            `;
        } else {
            foundItem.innerHTML = '<div>No restaurant found!</div>';
        }
